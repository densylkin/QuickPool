...
namespace QuickPool
{
    public class ObjectsPool : Singleton<ObjectsPool>
    {
        ...
        public Pool this[string name]
        {
            get
            {
                return pools.Where(pool => pool.poolName == name).FirstOrDefault();
            }
        }
        ...
        {
            Pool targetPool = ObjectsPool.Instance[name];
        ...
        {
            for (int i = 0; i < ObjectsPool.Instance.pools.Count; i++)
                ObjectsPool.Instance.pools[i].DespawnAll();
        }
    }
}